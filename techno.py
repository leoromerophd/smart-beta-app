...
def show():
    st.title(":minidisc: Technology :computer:")
    etf_name = "Technology"
    ...
    if product_number:
        ...
        if download_csv(product_number):
           ...
           # Botón para descargar datos históricos y mostrar rendimientos y riesgo
            if st.button(f'Cálculo de Rendimiento - Riesgo para el indice tecnología'):
                with st.spinner('... Calculando ...'):
                    start_date_str, end_date_str = calculate_dates()
                    historical_data = {ticker: yf.download(ticker, start=start_date_str, end=end_date_str) for ticker in top_15_tickers}
